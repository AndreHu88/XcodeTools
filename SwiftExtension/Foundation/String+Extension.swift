...
extension String {
    ...
    public var isPhoneNumber: Bool {
        ...
        let phoneRegex: String = "^((13[0-9])|(15[^4,\\D])|(18[0,0-9])|(17[0,0-9])|16[0-9]|19[0-9])\\d{8}$"
        let predicate = NSPredicate(format: "SELF MATCHES %@",phoneRegex)
        
        let chinesePhoneRegex: String = "^(\\+86)\\s?((13[0-9])|(15[^4,\\D])|(18[0,0-9])|(17[0,0-9])|16[0-9]|19[0-9])\\s?\\d{4}\\s?\\d{4}$?"
        let chinesePredicate = NSPredicate(format: "SELF MATCHES %@", chinesePhoneRegex)
        return predicate.evaluate(with: self) || chinesePredicate.evaluate(with: self)
    }
    ...
    public func height(_ width: CGFloat, font: UIFont, lineBreakMode: NSLineBreakMode = .byWordWrapping) -> CGFloat {
...
//        }
    ...
    public func width(_ height: CGFloat, font: UIFont) -> CGFloat {
...
extension String {
    ...
    func stringToClass() -> AnyClass? {
        
        // 获取swift 命名空间
        let nameSpace = Bundle.main.infoDictionary!["CFBundleExecutable"]
        if let currentNameSpace = nameSpace as? String {
            
            let currentClass: AnyClass? = NSClassFromString(currentNameSpace + "." + self)
            return currentClass
        }
        return nil
    }
    
    func stringToViewController() -> UIViewController? {
        ...
        if let currentNameSpace = nameSpace as? String {
            ...
            let currentClass: AnyClass? = NSClassFromString(currentNameSpace + "." + self)
            if let classType = currentClass as? UIViewController.Type {
    ...
    func setAttributedString(_ lineSpacing: CGFloat, _ textColor: UIColor, _ textFont: UIFont) -> NSAttributedString {
        
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = lineSpacing
        let attributedString = NSMutableAttributedString(string: self, attributes: [NSAttributedString.Key.paragraphStyle: paragraphStyle])
        
        attributedString.setAttributes([NSAttributedString.Key.foregroundColor: textColor, NSAttributedString.Key.font: textFont], range: NSMakeRange(0, self.length))
        return attributedString
    }
    
    func getSpaceLabelHeightwithSpeace(_ lineSpeace: CGFloat, _ font: UIFont, _ width: CGFloat) -> CGFloat {
        
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = lineSpeace
        let dic = [NSAttributedString.Key.font: font, NSAttributedString.Key.paragraphStyle: paragraphStyle]
        let size = self.boundingRect(with: CGSize(width: width, height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: dic, context: nil)
